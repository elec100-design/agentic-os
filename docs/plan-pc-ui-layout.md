...
1. 좌측 사이드바 **기본 240px**로 충분한가, 아니면 우측 레일처럼 더 넓은 기본(280px)을 원하는가.
2. 접기 모드는 **아이콘 전용 64px**과 **완전 숨김** 중 어느 쪽이 좋은가.
3. 첨부한 파일을 채널 작업 위치 안으로 **복사**할지, 지금처럼 `data/uploads/`에 두고
   경로만 알려줄지(현행은 후자 — CLI가 절대경로로 읽는다).
