...
### V4.7 — 커스텀 에이전트 + 에이전트 간 협업 (구현 완료, 2026-08-13)

Grok Bot 기능 이식 검토([plan-grok-bot-features.md](plan-grok-bot-features.md))의
A·B1·B2. "에이전트 = 벤더 CLI"였던 것을 **사용자가 만드는 역할**로 한 층 올린다.
승인 게이트(C)·루틴(D)·에이전트별 기억(E)은 다음 차수.

- **커스텀 에이전트**(`agents` 테이블, `/settings/agents`) — 이름·역할(페르소나)·
  실행 CLI·작업 위치·읽기 전용. 페르소나는 프롬프트 프리픽스로 주입되므로
  **벤더 중립**이고, 실행 CLI를 `auto`로 두면 역할은 고정된 채 잔여 쿼터
  라우팅이 그때그때 CLI를 고른다(이 앱만의 이점 — Grok Bot의 봇은 단일 모델).
  첫 실행에 프리셋 3종(researcher·builder·reviewer)을 한 번만 시드.
- **읽기 전용의 한계를 명시** — claude 만 `--permission-mode plan` + Bash 미허용으로
  실제 강제된다. 다른 CLI는 페르소나 문구로만 전달되는 **지시**다(`app/agents.py` 주석).
- **채널 @멘션** — 채널에 에이전트를 배치(`channel_members`)하고 `@슬러그`로 호출.
  아무 에이전트에도 없는 `@`는 그냥 텍스트, 실재하지만 멤버가 아니면 400.
- **에이전트 간 대화** — 에이전트 응답 안의 `@슬러그`가 다음 에이전트를 깨운다.
  훅 지점은 `worker._sync_message` 한 곳(잡의 모든 종료 경로가 지난다).
  **채널별 opt-in, 기본 꺼짐**(`channels.agent_chat`).
- **연쇄 한도가 기능의 본체** — 연쇄 1회 = CLI 실행 1회 = 구독 쿼터 소모다.
  hop 깊이(3)·쓰레드 총량(8)·왕복(2)·자기멘션 무시·한 응답당 첫 멘션 하나만
  (지수 확산 차단)·채널 킬 스위치. 걸리면 실패가 아니라 정지 + system 메시지.
- **세션 격리** — `latest_thread_session(agent_id=…)`. 한 쓰레드에 같은 CLI를 쓰는
  에이전트가 둘이면 A가 B의 세션을 물려받아 맥락을 통째로 흡수하던 버그를 막는다.
- **워크플로 동적 위임** — 태스크 출력의 `{"handoff": {...}}`를 회수해 실행 중인
  그래프에 새 태스크를 붙인다(`depends_on` = 위임한 태스크). 산출물에서 JSON은
  제거하고 저장한다(하류 프롬프트 오염 방지). 자기·상류로 되돌리는 위임은 거절,
  프로젝트당 `ORCH_MAX_DYNAMIC_TASKS`(3)개까지.

### V4.8 — 승인 게이트 (구현 완료, 2026-08-13)

Grok Bot 이식 검토의 C. 되돌릴 수 없는 동작을 사람이 승인한 뒤에 실행한다.
워크스페이스 파일 변경은 gitcheckpoint 가 되돌릴 수 있지만 **메일 발송·외부
쓰기·결제·삭제는 되돌릴 수 없다** — 지금까지 그쪽엔 방어가 전혀 없었다.

- **도구 단위 가로채기가 아니라 벤더 중립 "제안 → 승인 → 실행"** — 이 앱은
  Agent SDK 가 아니라 CLI 를 spawn 하므로 PreToolUse `defer` 를 쓸 수 없다.
  대신 에이전트가 제안을 JSON 으로 내면 approvals 행이 되고, 사람이 승인하면
  앱이 별도 잡으로 실행한다.
- **강제와 지시를 구분해 명시** — `approval_policy="ask"` 자체는 프롬프트로
  전달되는 **지시**다. 실제 강제는 도구를 주지 않는 데서 나온다:
  `read_only=1` + provider=claude(plan 모드·Bash 미허용) 조합만 실행 수단
  자체가 없다. UI 가 이 사실을 그대로 안내한다.
- **승인 시 게이트는 그 동작 하나에만 해제** — 실행 잡에는 게이트 지시도
  읽기 전용도 걸지 않는다. 그러지 않으면 승인해도 또 제안만 하고 끝난다.
- **제안 JSON 은 결과물에서 걷어낸다** — 수확이 `_sync_message` 보다 먼저
  돌아, 정리된 출력이 채널 메시지 본문·하류 태스크 프롬프트로 이어진다.
- **채널 제안은 결과도 그 쓰레드에 남는다** — 승인 실행은 원래 쓰레드의
  에이전트 메시지로, 거절은 system 메시지로. 대화 밖에서 조용히 실행되면
  무슨 일이 있었는지 기록에서 사라진다.
- 인박스 `/approvals` + 사이드바 대기 배지(20초 폴링),
  잡당 제안 상한 `AOS_APPROVAL_MAX_PER_JOB`(5).

### V4.9 — 에이전트 목록·DM·채널 소환 (구현 완료, 2026-08-13)

V4.7 에서 에이전트와 채널의 **알맹이**는 만들었지만 들어가는 문이 없었다 —
관리 화면(`/settings/agents`)뿐이라, 만든 에이전트에게 말을 걸 방법도 채널을
만들 방법도 UI 에 없었다. 이 단계가 그 문을 낸다.

- **좌측 사이드바가 대화 상대 목록이 된다** — 에이전트(아바타·이름·@슬러그·
  마지막 발화 미리보기)와 채널(이름·멤버 수·미리보기). 홈과 대화 페이지가 같은
  목록을 쓴다: 대화 중에도 다른 상대로 바로 옮겨갈 수 있어야 한다.
- **DM 도 채널로 구현** (`channels.kind='dm'`, `agent_id`) — 메시지·쓰레드·
  실행 트레이스·SSE 를 그대로 재사용한다. `/dm/{slug}` 이 없으면 만들고 연다(멱등).
  DM 에서는 **멘션 없이** 보낸 말이 그 에이전트에게 간다. 상대가 한 명뿐인 방에서
  이름을 부르게 하는 건 번거롭기만 하다. 멤버 추가·에이전트 간 대화·CLI 선택은
  DM 화면에서 감춘다(의미가 없다).
- **채널 만들기 화면** (`/channels/new`) — 이름·주제·작업 위치에 더해 **부를
  에이전트를 체크로 소환**하고 에이전트 간 대화 허용을 함께 정한다.
- **미리보기는 상태로 물러선다** — 마지막 메시지가 실행 중이면 "실행 중…",
  실패해 본문이 없으면 `실패 · <직전 발화>`. 빈 줄을 두면 대화가 없는 것처럼 보인다.
- 에이전트를 보관하면 그 DM 도 목록에서 내려간다(대화 기록 자체는 남는다).
- **대화는 홈 중앙의 탭으로 열린다** — 목록에서 고르면 페이지를 옮기는 대신
  작업·비전보드 탭과 나란히 뜬다. 그래서 대화 화면을 조각
  (`partials/channel_view.html`)으로 떼고 `channels.js` 를
  `mountChannelView(root)` 로 바꿨다: 탭을 여러 개 열면 id 가 겹치므로 전역
  id 대신 root 안쪽만 보고, 탭을 닫으면 열어 둔 EventSource 를 끊는다.
  전용 페이지(`/channels/{id}`)도 같은 조각을 써서 구현이 하나로 남는다.
  (이 과정에서 `/api/messages/{id}/thread` 가 `{root_id, messages}` 를 주는데
  옛 코드가 응답을 배열로 순회하던 버그를 발견해 함께 고쳤다 — 쓰레드 답장
  패널이 열리지 않던 원인이다.)
