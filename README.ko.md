...
- **에이전트 목록·1:1 대화·채널** — 좌측 사이드바가 대화 상대 목록이다. 에이전트마다 마지막 발화가, 채널마다 멤버 수가 보인다. 에이전트를 누르면 1:1 대화(DM)로 가고 거기서는 `@` 없이 그냥 말을 걸면 된다. 채널을 만들 때는 부를 에이전트를 체크로 소환한다
- **커스텀 에이전트** — **⚙︎ 에이전트** 에서 역할(페르소나)·실행 CLI·작업 위치·읽기 전용을 가진 에이전트를 만든다. 실행 CLI를 「자동」으로 두면 역할은 고정된 채 매 실행이 사용량이 가장 여유 있는 CLI로 간다. 읽기 전용은 claude 에서만 실제로 강제되고(plan 모드·Bash 미허용), 다른 CLI 에는 지시로만 전달된다
- **에이전트끼리 대화·일 넘기기** — 채널에 에이전트를 배치하고 `@슬러그`로 부른다. 「에이전트 간 대화 허용」을 켜면 에이전트가 답변에서 다른 에이전트를 `@`로 불러 일을 넘길 수 있다. **기본은 꺼짐**이고, 연쇄는 깊이·발화 수·왕복 상한에 걸리면 이유를 남기고 멈춘다 — 무한 연쇄는 곧 구독 쿼터 소각이기 때문(`AOS_AGENT_HOP_MAX`·`AOS_AGENT_CHAIN_MAX`·`AOS_AGENT_PINGPONG_MAX`)
- **승인 게이트** — 에이전트에 「되돌릴 수 없는 동작은 승인받고 실행」 정책을 주면 메일 발송·외부 쓰기·결제·삭제를 직접 하지 않고 제안만 한다. 제안은 **✋ 승인** 인박스에 쌓이고, 승인하면 그 에이전트가 그 동작만 실행한다(게이트가 그것 하나에만 풀린다). 다만 이 정책 자체는 프롬프트로 전달되는 **지시**다 — 실제로 막는 것은 도구를 주지 않는 것이라, 읽기 전용 + claude 조합(plan 모드·Bash 미허용)으로 함께 쓰면 실행 수단 자체가 없어진다
...
- **비전 보드** — 홈 우측 레일의 **프로젝트** 탭에서 프로젝트 목표를 주면 메인 오케스트레이터가 의존성 순서 DAG로 분해합니다. (기존 `/board` URL은 홈으로 리다이렉트)
...
- **채팅 레일 검색·정렬·이름 변경** — 홈 채팅 레일에서 이름·에이전트·작업 위치로 필터, 시간순/프로젝트별 정렬, 말풍선 ✎로 세션 이름을 그 자리에서 바꿔 프롬프트 대신 라벨로 표시
...
- **파일 첨부** — 드래그앤드롭으로 사진·파일을 프롬프트에 첨부(크기 한도 설정 가능).
  에이전트 1:1 대화(DM)·채널 대화창도 같은 컴포저를 쓴다 — 파일 첨부, 에이전트·모델 선택,
  그리고 답변이 실행될 폴더/GitHub 리포 선택. 고른 폴더는 그 대화방의 기본값으로 남는다
- **좌측 사이드바 크기 조절** — 180~360px 사이로 끌어서 맞추거나 64px 아이콘 레일로 접을 수 있고, 폭은 기억된다
...
1. 홈 우측 레일의 **프로젝트** 탭을 엽니다 (기존 `/board` URL은 홈으로 리다이렉트)
...
│   ├── agents.py        # 커스텀 에이전트 — 역할 주입·@멘션·에이전트 간 연쇄 한도
│   ├── approvals.py     # 승인 게이트 — 제안 → 사람이 승인 → 실행
